...
export const QuoteTable: React.FC<QuoteTableProps> = ({
  ...
  const renderProcessingTimeCell = (r: QuoteRequest) => {
    if (!r.acceptedAt) {
      return <span className="text-faint text-[15px]">Chưa tiếp nhận</span>;
    }
    ...
    return (
      <div ref={ref} className="relative inline-block" onClick={(e) => e.stopPropagation()}>
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-[4px] min-w-[118px] py-[4px] px-[10px] rounded-[20px] text-[15.5px] font-bold cursor-pointer whitespace-nowrap shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-[all_0.12s]"
          // động — giữ inline
          style={{
          ...
          >
            {options.map((opt) => (
              <button
                ...
                className={clsx(
                  'flex items-center gap-[8px] w-full py-[7px] px-[10px] border-0 rounded-[7px] text-[16.5px] cursor-pointer text-left',
    ...
    if (!expanded) {
      return (
        ...
        >
          ...
          <span className="text-faint font-extrabold text-[17px]">...</span>
        </button>
      );
    }
  ...
  return (
    <div className="w-full overflow-x-auto border border-border rounded-[10px]">
      <table className="w-full min-w-[1500px] border-collapse text-[16.5px] [&_th]:text-left [&_th]:py-[6px] [&_th]:px-[10px] [&_th]:text-[15px] [&_th]:font-bold [&_th]:uppercase [&_th]:text-muted [&_th]:border-b [&_th]:border-border [&_th]:bg-[#f8fafc] [&_th]:whitespace-nowrap [&_th]:sticky [&_th]:top-0 [&_th]:z-[1] [&_td]:py-[6px] [&_td]:px-[10px] [&_td]:border-b [&_td]:border-[#f1f5f9] [&_td]:align-middle [&_td]:whitespace-nowrap [&_tr]:cursor-pointer [&_tr]:transition-[background] [&_tr]:duration-150 [&_tr:hover]:bg-[#f8fafc]">
        <thead>
          <tr>
            <th>Mã Hỏi Giá</th>
            <th>Thời Gian Tạo</th>
              ...
              >
                <td>
                  <span className="inline-flex items-center gap-[6px]">
                    <strong className="font-mono text-[16px] text-[#1e293b]">{r.code || r.id}</strong>
                    {canChat && (
                      <button
                        type="button"
                      ...
                      >
                        ...
                        {unread > 0 && (
                          <span className="absolute top-[-4px] right-[-5px] bg-[#ef4444] text-white rounded-full text-[13px] font-extrabold min-w-[13px] h-[13px] leading-[13px] text-center px-[2px]">
                            {unread > 9 ? '9+' : unread}
                          </span>
                        )}
                      </button>
                ...
                <td className="text-muted text-[15px]">
                  {r.createdAt
                ...
                <td>
                  <span className="bg-[#f1f5f9] text-[#475569] py-[3px] px-[8px] rounded-[6px] text-[15px] font-semibold">
                    {r.category?.name || '---'}
                  </span>
                </td>
                <td>
                  <div className="relative inline-block">
                    ...
                    {r.images && r.images.length > 1 && (
                      <span className="absolute bottom-[-2px] right-[-2px] bg-[#0f172a] text-surface text-[13.5px] font-extrabold py-[1px] px-[4px] rounded-[4px] border border-surface pointer-events-none">
                        +{r.images.length - 1}
                      </span>
                    )}
                  </div>
                ...
                {isRejected ? (
                  ...
                  >
                    <div className="inline-flex items-center gap-[5px]">
                      ...
                      {r.rejectReason && (
                        <span className="text-[14px] bg-[#ffe4e6] text-[#9f1239] border border-[#fecdd3] py-[1px] px-[5px] rounded-[4px] cursor-help">
                          ⓘ Lý do
                        </span>
                      )}
                    </div>
                  ...
                  (r.status === 'QUOTED' || r.status === 'CLOSED') ? (
                    <td className="text-[#0f766e] rounded-[6px] py-[6px] px-[8px] font-extrabold text-[17px]">
                    ...
                    >
                      <div className="inline-flex flex-col items-start">
                        <span className="text-faint italic font-bold text-[16.5px] opacity-75">
                          {formattedPrice}
                        </span>
                        {renderPriceBreakdownLines(priceBd)}
                        <span className="text-[13.5px] text-[#ea580c] bg-[#fff7ed] border border-[#ffedd5] py-0 px-[4px] rounded-[3px] font-extrabold mt-[2px] leading-[14px]">
                          Chưa duyệt
                        </span>
                      </div>
                    </td>
                ...
                <td className="text-[16px] font-semibold text-[#334155]">{r.customerMeasurements || '---'}</td>
                {!isCompactView && (
                  <td className="text-[15px] text-[#475569] font-semibold">
                    {r.closeRatePct !== undefined && r.closeRatePct !== null ? `${r.closeRatePct}%` : '---'}
                  </td>
                )}
                {!isCompactView && (
                  <td className="text-[15px] text-[#d97706] max-w-[160px] overflow-hidden text-ellipsis font-semibold">
                    {displayNote}
                  </td>
                )}
                {!isCompactView && (
      ...
      {zoomedImage && createPortal(
        ...
        >
          ...
          <span className="fixed bottom-[20px] left-1/2 -translate-x-1/2 text-[rgba(255,255,255,0.7)] text-[16px] font-semibold">
