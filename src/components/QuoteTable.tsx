...
export const QuoteTable: React.FC<QuoteTableProps> = ({
  ...
  return (
    <div className="w-full overflow-x-auto border border-border rounded-[10px]">
      <table className="w-full min-w-[1500px] border-collapse text-[15.5px] [&_th]:text-left [&_th]:py-[6px] [&_th]:px-[10px] [&_th]:text-[14px] [&_th]:font-bold [&_th]:uppercase [&_th]:text-muted [&_th]:border-b [&_th]:border-border [&_th]:bg-[#f8fafc] [&_th]:whitespace-nowrap [&_th]:sticky [&_th]:top-0 [&_th]:z-[1] [&_td]:py-[6px] [&_td]:px-[10px] [&_td]:border-b [&_td]:border-[#f1f5f9] [&_td]:align-middle [&_td]:whitespace-nowrap [&_tr]:cursor-pointer [&_tr]:transition-[background] [&_tr]:duration-150 [&_tr:hover]:bg-[#f8fafc]">
        <thead>
          <tr>
            <th>Mã Hỏi Giá</th>
            <th>Thời Gian Tạo</th>
        ...
        <tbody>
          {requests.map((r) => {
            ...
            return (
              ...
              >
                <td><strong className="font-mono text-[15px] text-[#1e293b]">{r.code || r.id}</strong></td>
                ...
                {!isCompactView && (
                  <td className="text-[14px] text-[#d97706] max-w-[160px] overflow-hidden text-ellipsis font-semibold">
                    {displayNote}
                  </td>
                )}
                {!isCompactView && (
