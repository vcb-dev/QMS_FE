...
export const DistributionPieCard: React.FC<DistributionPieCardProps> = ({ title, subtitle, data }) => (
  <div className={cardCls}>
    <h2 className="text-[18px] font-extrabold text-[#0f172a] m-0 mb-[4px]">{title}</h2>
    <span className="text-[15px] text-[#334155]">{subtitle}</span>
          ...
          {data.slice(0, 5).map((d, i) => (
            <div key={i} className="flex items-center justify-between gap-[8px]">
              <div className="flex items-center gap-[6px] min-w-0">
                ...
                <span className="text-[15px] text-[#475569] font-semibold overflow-hidden text-ellipsis whitespace-nowrap">{d.name}</span>
              </div>
              <span className="text-[16px] font-black text-[#0f172a] shrink-0">{d.value}</span>
            </div>
          ))}
        </div>
      </div>
    ) : (
      <div className="text-center text-[#64748b] text-[16.5px] py-[24px]">Chưa có dữ liệu</div>
    )}
  </div>
);
