...
export const SpecBadge: React.FC<SpecBadgeProps> = ({ icon, label, value, fullWidth, valueStyle, title }) => (
  ...
  >
    <span className="text-[13.5px] font-extrabold text-[#334155] uppercase flex items-center gap-[5px]">
      {icon} {label}
    </span>
    {/* động — giữ inline */}
    <strong className="text-[16px] text-[#0f172a] mt-[4px] block" style={valueStyle} title={title}>
      {value}
    </strong>
  </div>
);
