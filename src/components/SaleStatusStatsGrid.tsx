...
export const SaleStatusStatsGrid: React.FC<SaleStatusStatsGridProps> = ({ onSelectStatus }) => {
  ...
  const renderChange = (currVal: number, prevVal: number) => {
    if (prevVal === 0) {
      if (currVal === 0) return <span className="text-[15px] font-bold text-[#64748b]">0%</span>;
      return (
        <span className="inline-flex items-center gap-[3px] text-[15px] font-extrabold text-[#2563eb]">
    ...
    return (
      <span
        className={clsx(
          "inline-flex items-center gap-[3px] text-[15px] font-extrabold",
    ...
    >
      <div className="flex items-center justify-between mb-[16px]">
        ...
        <div className="flex border border-[#cbd5e1] rounded-[8px] overflow-hidden">
          <button
            ...
            className={clsx(
              "px-[14px] py-[6px] text-[16px] font-bold border-none cursor-pointer",
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] gap-[12px]">
        {STATUS_ITEMS.map((item) => (
          ...
          >
            <div className="text-[15px] font-bold text-[#334155] uppercase tracking-[0.3px] mb-[6px]">
              {item.label}
            </div>
            <div className="text-[28px] font-black text-[#0f172a] mb-[4px]">
              {current[item.key]}
            </div>
            <div className="flex items-center gap-[5px]">
              {renderChange(current[item.key] ?? 0, previous[item.key] ?? 0)}
              <span className="text-[14.5px] text-[#64748b]">so với {periodLabel}</span>
            </div>
          </div>
        ))}
      </div>
