...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const [prevPeriodRequests, setPrevPeriodRequests] = React.useState<QuoteRequest[] | null>(null);
  ...
  const handleTimeRangeChange = async (newRange: string) => {
    ...
    try {
      ...
      if (currentRole === 'ADMIN') {
        ...
        if (prevQuery) {
          const prevRes = await fetchQuoteRequests({ ...prevQuery, limit: 500 });
          setPrevPeriodRequests(prevRes?.data || []);
        } else {
          setPrevPeriodRequests(null);
        }
      }
    } catch (err) {
  ...
  const timelineChartData = React.useMemo(() => {
    ...
    const increment = (b: any, status: string) => {
      ...
      else if (status === 'DA_CHOT') b.daChot += 1;
    };
    ...
    if (timeRange === 'TODAY') {
      ...
      slots.forEach((s) => {
        const b = { key: s.label, label: s.label, ycMoi: 0, dangXly: 0, needMoreInfo: 0, xong: 0, tuChoi: 0, daChot: 0, total: 0, value: 0 };
        map.set(s.label, b);
        buckets.push(b);
      });
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const b = { key, label, ycMoi: 0, dangXly: 0, needMoreInfo: 0, xong: 0, tuChoi: 0, daChot: 0, total: 0, value: 0 };
        map.set(key, b);
        buckets.push(b);
      }
    ...
    return buckets.map((b) => {
      ...
      else if (chartStatusFilter === 'DA_CHOT') val = b.daChot;
  ...
  const sumRevenue = (list: QuoteRequest[]) => {
    ...
    list.forEach((r) => {
      ...
      if (r.status === 'DA_CHOT') closedRevenue += price;
      else if (r.status === 'XONG') quotedRevenue += price;
    });
  ...
  const prevRevenueStats = React.useMemo(
    () => (prevPeriodRequests ? sumRevenue(prevPeriodRequests) : null),
    [prevPeriodRequests]
  );
  ...
  const sumKpi = (list: QuoteRequest[]) => {
    ...
    const closed = list.filter((r) => r.status === 'DA_CHOT').length;
  ...
  const prevKpiStats = React.useMemo(
    () => (prevPeriodRequests ? sumKpi(prevPeriodRequests) : null),
    [prevPeriodRequests]
  );
  ...
  const saleStatsData = React.useMemo(() => {
    ...
    apiRequests.forEach((r) => {
      ...
      b.total += 1;
      if (r.status === 'DA_CHOT') b.closed += 1;
    });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
      ...
      {currentRole === 'SALE' ? (
        ...
        const chartData = [
          ...
          { name: 'Đã chốt',     value: counts.daChot,        fill: '#8b5cf6' },
        ];
            ...
            return (
              <div style={{ background: '#0f172a', color: '#fff', padding: '10px 14px', borderRadius: 8, fontSize: 12, fontWeight: 700, boxShadow: '0 4px 12px rgba(0,0,0,0.25)', minWidth: '150px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3, fontSize: 11.5 }}>
                  ...
                  <div style={{ color: '#a78bfa' }}>● Đã chốt: {dataItem.daChot}</div>
                </div>
              </div>
            );
          }
                ...
                <BarChart data={timelineChartData} margin={{ top: 18, right: 10, left: -20, bottom: 4 }} barCategoryGap="20%">
                  ...
                  {chartStatusFilter === 'ALL' ? (
                    <>
                      ...
                      <Bar dataKey="tuChoi" stackId="a" fill="#ef4444" name="Từ chối" />
                      <Bar dataKey="daChot" stackId="a" fill="#8b5cf6" name="Đã chốt" radius={[4, 4, 0, 0]} />
                    </>
                  ) : (
