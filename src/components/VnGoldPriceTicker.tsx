    ...
    <div className="bg-surface border border-border rounded-[14px] py-[18px] px-[22px] shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
      <div className="flex items-center justify-between mb-[14px] gap-[10px] flex-wrap">
        <div className="flex items-center gap-[10px] flex-wrap">
          <span className="text-[16.5px] font-black text-[#d97706] uppercase tracking-[0.6px]">
            Giá Vàng Thị Trường
          </span>
          <span className="inline-flex items-center gap-[5px] text-[13.5px] font-extrabold text-[#16a34a] tracking-[0.5px]">
            <span className="w-[6px] h-[6px] rounded-full bg-[#16a34a] animate-[livePulse_1.4s_ease-in-out_infinite]" />
            TRỰC TIẾP
          </span>
          <span className="text-[14.5px] text-[#64748b]">
      ...
      {error && <div className="text-[15.5px] text-[#dc2626]"> {error}</div>}
      {!error && items.length === 0 && (
        <div className="text-[15.5px] text-[#64748b]">Đang tải giá vàng thị trường...</div>
      )}
        ...
        <div className="grid [grid-template-columns:repeat(auto-fit,minmax(140px,1fr))] gap-0">
          {items.map((it, idx) => {
            ...
            return (
              ...
              >
                <div className="text-[15px] font-bold text-[#334155] whitespace-nowrap tracking-[0.2px]">
                  {cleanLabel(it.label)}
                </div>
                <div className="text-[24px] font-black text-[#0f172a] mt-[4px] [font-variant-numeric:tabular-nums] whitespace-nowrap">
                  {formatNumberVN(it.priceVnd)} <span className="text-[16px] font-bold text-[#334155] ml-[2px]">VNĐ</span>
                </div>
                <div
                  className={clsx(
                    'flex items-center gap-[4px] mt-[4px] text-[15.5px] font-extrabold',
