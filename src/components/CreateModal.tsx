...
const SelectedChip: React.FC<{ label: string; onRemove?: () => void; removeTitle?: string }> = ({ label, onRemove, removeTitle }) => (
  <span className="bg-[#f1f5f9] border border-[#cbd5e1] text-[#334155] py-[4px] px-[10px] rounded-[16px] text-[12.5px] font-bold inline-flex items-center gap-[6px]">
    ✓ {label}
    {onRemove && (
      <button
        type="button"
        onClick={onRemove}
        className="bg-transparent border-0 text-[#334155] cursor-pointer p-0 text-[13px] leading-[1] font-extrabold"
        title={removeTitle}
      >
        ✕
      </button>
      ...
      >
        ...
        <div className="shrink-0 bg-surface text-[#0f172a] py-[18px] px-[24px] flex items-center justify-between border-b border-border">
          <div className="flex items-center gap-[14px]">
            ...
            />
            <div>
              <h2 id="modalCreateTitle" className="text-[19px] font-extrabold m-0 text-[#0f172a]">
                Tạo Yêu Cầu Báo Giá Chế Tác Mới
              </h2>
              <p className="text-[13px] text-muted mt-[2px] mr-0 mb-0 ml-0">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden m-0">
          ...
          <div className="flex-1 overflow-y-auto bg-[#f8fafc] p-[20px]">
            ...
            <div className="bg-surface border border-border rounded-[16px] p-[20px] shadow-[0_1px_3px_rgba(0,0,0,0.03)] flex flex-col gap-[16px]">
              <h3 className="text-[14px] font-extrabold text-[#334155] m-0 uppercase tracking-[0.5px]">
                THÔNG TIN ĐƠN HÀNG
              </h3>
              ...
              <div className={formGroupCls}>
                ...
                <div ref={materialDropdownRef} className="relative">
                  ...
                  {materialDropdownOpen && (
                    ...
                    >
                      {materials.length === 0 && (
                        <div className="p-[8px] text-[13px] text-faint">Chưa có chất liệu nào</div>
                      )}
                      {materials.map((m) => {
                        const blocked =
                        ...
                        return (
                          <label
                            ...
                            className={clsx(
                              'flex items-center gap-[8px] py-[7px] px-[8px] rounded-[6px] text-[14px] font-semibold text-[#334155]',
                    ...
                    >
                        ...
                        {(['MAIN', 'SIDE'] as const).map((t) => {
                          ...
                          return (
                            <button
                              ...
                              className={clsx(
                                'flex-1 flex items-center justify-center gap-[6px] py-[7px] px-[10px] rounded-[7px] text-[14px] font-bold cursor-pointer transition-[background_0.12s,color_0.12s]',
                            ...
                            >
                              ...
                              <span
                                className={clsx(
                                  'text-[12px] font-bold py-[1px] px-[6px] rounded-full',
                      ...
                      {selectedStoneTypes.length > 0 && (
                        <div className="py-[8px] px-[10px] border-b border-border">
                          <input
                            ...
                            className="w-full py-[7px] px-[10px] text-[14px] border border-[#cbd5e1] rounded-[6px] outline-none"
                          />
                        </div>
                      )}
                      ...
                      <div className="flex-1 min-h-0 overflow-y-auto p-[6px]">
                        ...
                        {visibleStoneOptions.length > 0 && (
                          <label
                            className="flex items-center gap-[8px] py-[7px] px-[8px] rounded-[6px] text-[13px] font-bold text-[#334155] cursor-pointer"
                          >
                            <input
                              type="checkbox"
                        ...
                        {visibleStoneOptions.map((s) => (
                          <label
                            key={s.id}
                            className="flex items-center gap-[8px] py-[7px] px-[8px] rounded-[6px] text-[14px] font-semibold text-[#334155] cursor-pointer"
            ...
            <div className="bg-surface border border-border rounded-[16px] p-[20px] shadow-[0_1px_3px_rgba(0,0,0,0.03)] flex flex-col gap-[16px]">
              ...
              <div className={clsx(formGroupCls, 'flex-1')}>
                ...
                >
                  ...
                  <div className="font-bold text-[14px] text-[#0f172a] mb-[2px]">
                    {totalImageCount >= MAX_IMAGES
                  ...
                  <div className="flex flex-wrap gap-[10px] mt-[12px]">
                    {existingImageUrls.map((url, idx) => (
                      <div key={`existing-${idx}`} className="relative inline-block">
                        ...
                        <button
                          ...
                          className="absolute -top-[6px] -right-[6px] bg-[#ef4444] text-surface border-0 rounded-full w-[20px] h-[20px] text-[12px] font-extrabold cursor-pointer flex items-center justify-center shadow-[0_2px_4px_rgba(0,0,0,0.2)]"
                          title="Xóa ảnh này"
                        >
                          ✕
                        </button>
                    ...
                    {totalImageCount < MAX_IMAGES && (
                      <div
                        ...
                        className="w-[76px] h-[76px] rounded-[10px] border-2 border-dashed border-[#cbd5e1] bg-[#f8fafc] flex flex-col items-center justify-center cursor-pointer text-[#475569] text-[12px] font-extrabold gap-[2px]"
                ...
                {existingVideoUrl || newVideoPreviewUrl ? (
                  <div className="relative inline-block mt-[4px]">
                    ...
                    <button
                      type="button"
                      onClick={removeVideo}
                      className="absolute -top-[6px] -right-[6px] bg-[#ef4444] text-surface border-0 rounded-full w-[22px] h-[22px] text-[13px] font-extrabold cursor-pointer flex items-center justify-center shadow-[0_2px_4px_rgba(0,0,0,0.2)]"
                      title="Xóa video này"
                    >
                      ✕
                    </button>
                  ...
                  >
                    ...
                    <div className="font-bold text-[14px] text-[#0f172a] mt-[4px]">
                      Bấm để chọn 1 video
                    </div>
                    <span className="text-[12px] text-muted">
              ...
              <div className="bg-[#f8fafc] border border-border rounded-[10px] p-[14px] text-[13.5px] text-[#475569] leading-[1.5]">
              ...
              <label className="font-bold cursor-pointer flex items-center gap-[8px] text-[14px] text-[#334155]">
                <input
                  type="checkbox"
          ...
          <div className="shrink-0 bg-surface border-t border-[#cbd5e1] py-[14px] px-[24px] flex items-center justify-end gap-[12px] shadow-[0_-4px_16px_rgba(0,0,0,0.05)]">
            <button
              type="button"
              onClick={onClose}
              className="bg-[#f1f5f9] border border-[#cbd5e1] rounded-[10px] py-[11px] px-[22px] text-[14.5px] font-bold text-[#475569] cursor-pointer"
            >
              Hủy bỏ
            </button>
            <button
              ...
              disabled={submitting}
              className={clsx(
                'bg-[#e2e8f0] text-[#0f172a] border border-[#94a3b8] rounded-[10px] py-[12px] px-[28px] text-[15px] font-extrabold cursor-pointer flex items-center justify-center gap-[8px] shadow-none',
