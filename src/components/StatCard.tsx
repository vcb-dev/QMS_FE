...
export const StatCard: React.FC<StatCardProps> = ({ icon, label, value, tone = 'default', deltaPct, deltaLabel = 'so với tháng trước' }) => {
  ...
  return (
    <div className={clsx("border border-solid rounded-[14px] py-[18px] px-[20px] shadow-[0_1px_3px_rgba(0,0,0,0.03)]", t.card)}>
      <div className={clsx("flex items-center gap-[8px] text-[12px] font-extrabold uppercase", t.label)}>
        {icon}
        {label}
      </div>
      <div className={clsx("text-[25px] font-black mt-[6px]", t.value)}>{value}</div>
      {hasDelta && (
        <div className={clsx("text-[12px] font-bold mt-[6px]", deltaCls)}>{deltaText}</div>
      )}
    </div>
  );
};
