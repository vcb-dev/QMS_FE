...
const STATUS_META: Record<string, { label: string; color: string; bg: string }> = {
  YC_MOI: { label: 'Mới tạo', color: '#1d4ed8', bg: '#eff6ff' },
  DANG_XLY: { label: 'Đang xử lý', color: '#b45309', bg: '#fffbeb' },
  NEED_MORE_INFO: { label: 'Cần bổ sung', color: '#c2410c', bg: '#fff7ed' },
  XONG: { label: 'Đã báo giá', color: '#15803d', bg: '#f0fdf4' },
  TU_CHOI: { label: 'Từ chối', color: '#be123c', bg: '#fff1f2' },
  DA_CHOT: { label: 'Đã chốt', color: '#6d28d9', bg: '#f5f3ff' },
};
...
export const CustomerManagementView: React.FC = () => {
  ...
  const customerStats = useMemo(() => {
    ...
    return customers.map((c) => {
      ...
      const closedOrders = orders.filter((o) => o.status === 'DA_CHOT');
                            ...
                            .map((o) => {
                              ...
                              return (
                                <div key={o.id} style={{ display: 'grid', gridTemplateColumns: '110px 1.8fr 1fr 1fr 1fr', gap: '10px', alignItems: 'center', fontSize: '12px', padding: '6px 0', borderBottom: '1px solid #f1f5f9' }}>
                                  ...
                                  <span style={{ fontWeight: 700, color: o.status === 'DA_CHOT' ? '#15803d' : '#334155' }}>
