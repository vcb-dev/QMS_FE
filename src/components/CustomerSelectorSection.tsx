      ...
      <div className="flex items-center justify-between mb-[6px]">
        ...
        {isNewCustomerMode && (
          <button
            ...
            className="py-[4px] px-[10px] text-[12px] font-bold rounded-[6px] border border-[#cbd5e1] bg-surface text-muted cursor-pointer flex items-center gap-[4px]"
          >
      ...
      {!isNewCustomerMode ? (
        <div className="flex flex-col gap-[6px]">
          <div className="relative" ref={wrapRef}>
            ...
            <input
              type="text"
              className={clsx(formControlCls, '!pl-[30px] !text-[13px]')}
            ...
            {isDropdownOpen && (
              ...
              >
                {customerSearchLoading && (
                  <div className="p-[10px] text-[13px] text-faint">Đang tìm...</div>
                )}
                {!customerSearchLoading && customerList.length === 0 && (
                  <div className="py-[6px] px-[10px] text-[13px] text-faint">Không tìm thấy khách hàng nào</div>
                )}
                {!customerSearchLoading && customerList.map((cust) => {
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        'w-full flex items-center justify-between gap-[8px] py-[8px] px-[10px] rounded-[8px] border-0 text-[#0f172a] text-[13px] cursor-pointer text-left',
                ...
                {!customerSearchLoading && (
                  <div className={clsx('p-[6px]', customerList.length > 0 && 'border-t border-border mt-[2px] pt-[8px]')}>
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-[6px] py-[8px] px-[10px] rounded-[8px] border-[1.5px] border-dashed border-[#10b981] bg-[#f0fdf4] text-[#15803d] text-[13px] font-bold cursor-pointer"
                    >
                      <UserPlus size={13} />
                      {customerSearch.trim()
          ...
          {selectedCustomer && !isDropdownOpen && (
            <div className="flex items-center gap-[6px] py-[6px] px-[10px] bg-[#eff6ff] rounded-[8px] text-[12.5px] text-[#1e40af] font-semibold">
          ...
          {!customerSearchLoading && customerList.length === 0 && (
            <button
              ...
              className="flex items-center justify-center gap-[6px] py-[8px] px-[10px] rounded-[8px] border-[1.5px] border-dashed border-[#10b981] bg-[#f0fdf4] text-[#15803d] text-[13px] font-bold cursor-pointer"
            >
              <UserPlus size={13} />
              {customerSearch.trim()
