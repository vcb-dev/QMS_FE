...
const PriceTooltip: React.FC<any> = ({ active, payload }) => {
  ...
  return (
    <div className="bg-surface border border-border rounded-[8px] py-[8px] px-[10px] text-[13px] shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
      <div className="font-extrabold text-[#0f172a]">{d.toLocaleDateString('vi-VN')}</div>
      <div className="text-[12px] text-faint mb-[4px]">{d.toLocaleTimeString('vi-VN')}</div>
    ...
    <div className={modalBackdropCls} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-[10px] flex-wrap py-[14px] px-[20px] border-b border-border shrink-0">
          <div className="flex gap-[4px]">
            {QUICK_RANGES.map((opt) => (
              <button
                ...
                className={clsx(
                  'py-[6px] px-[12px] rounded-[6px] border-0 text-[12.5px] font-bold cursor-pointer',
          ...
          />
          <span className="text-[12px] font-bold text-faint">đến</span>
          <input
            type="date"
            value={endDate}
          ...
          {quickRange !== 'THIS_MONTH' && (
            <button
              ...
              className="bg-transparent border-0 text-[#b91c1c] text-[12.5px] font-bold cursor-pointer"
            >
              Xóa bộ lọc
            </button>
          )}
        ...
        {!loading && !error && chartData.length > 0 && trend && (
          <div className="pt-[16px] px-[20px] pb-[4px] shrink-0">
            <div className="flex items-start justify-between flex-wrap gap-[10px] mb-[6px]">
              <div>
                <div className="text-[12px] font-bold text-faint uppercase tracking-[0.3px]">
                  {baseMetals.find((m) => m.id === metalFilter)?.name || ''}
                </div>
                <div className="flex items-baseline gap-[10px] mt-[2px]">
                  <span className="text-[23px] font-black text-[#0f172a] [font-variant-numeric:tabular-nums]">
                    {formatCurrency(trend.last)}
                  </span>
                  <span
                    className="inline-flex items-center gap-[2px] text-[13.5px] font-bold"
                    // động — giữ inline
              ...
              <div className="flex gap-[4px]">
                {baseMetals.map((m) => (
                  <button
                    ...
                    className={clsx(
                      'py-[5px] px-[11px] rounded-[6px] border-0 text-[12.5px] font-bold cursor-pointer',
        ...
        <div className={clsx(modalBodyCls, 'flex-1 min-h-0 overflow-auto !p-0 block')}>
          {loading && <p className="text-[13.5px] text-muted p-[20px]">Đang tải...</p>}
          {error && <p className="text-[13.5px] text-[#dc2626] p-[20px]">{error}</p>}
          {!loading && !error && filteredRows.length === 0 && (
            <p className="text-[13.5px] text-faint p-[20px]">Không có lịch sử thay đổi giá phù hợp bộ lọc.</p>
          )}
          {!loading && !error && filteredRows.length > 0 && (
            <table className="w-full min-w-[620px] table-fixed border-collapse text-[13.5px]">
              <colgroup>
                <col className="w-[20%]" />
                <col className="w-[17%]" />
                <col className="w-[14%]" />
                  ...
                  <tr key={r.id} className="border-t border-[#f1f5f9]">
                    ...
                    <td className="py-[10px] px-[12px] text-right">
                      {r.changePct != null && r.changePct !== 0 ? (
                        <span className={clsx(
                          'inline-flex items-center gap-[2px] text-[12.5px] font-bold',
                    ...
                    <td className="py-[10px] px-[12px] text-faint text-[12.5px] overflow-hidden text-ellipsis whitespace-nowrap">
                      {new Date(r.createdAt).toLocaleString('vi-VN')}
                    </td>
                    <td className="py-[10px] px-[20px] text-faint text-[12.5px] overflow-hidden text-ellipsis whitespace-nowrap">
