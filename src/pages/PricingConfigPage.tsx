...
const SectionHeader: React.FC<{ title: string; action?: React.ReactNode; icon?: LucideIcon }> = ({ title, action, icon: Icon }) => (
  <div className="flex items-center justify-between flex-wrap gap-[10px] mb-[14px]">
    <div className="flex items-center gap-[8px] min-w-0">
      ...
      <h3 className="text-[15.5px] font-extrabold text-[#0f172a] m-0 [word-break:keep-all]">{title}</h3>
    </div>
    {action}
  </div>
);
...
const RuleCard: React.FC<{ title: string; subtitle?: string; action?: React.ReactNode; children: React.ReactNode; icon?: LucideIcon }> = ({ title, subtitle, action, children, icon }) => (
  <div className="border border-[#e5e7eb] rounded-[12px] p-[18px]">
    <SectionHeader title={title} action={action} icon={icon} />
    {subtitle && <p className="text-[13.5px] text-[#64748b] -mt-[6px] mr-0 mb-[12px] ml-0">{subtitle}</p>}
    {children}
  </div>
);
...
const ErrorBanner: React.FC<{ message: string; className?: string }> = ({ message, className }) => (
  <div className={clsx('flex items-start gap-[6px] text-[#b91c1c] text-[13.5px] bg-[#fef2f2] border border-[#fca5a5] py-[8px] px-[10px] rounded-[8px]', className)}>
...
const ValueDisplay: React.FC<{ value: number; unit?: string; dirty?: boolean }> = ({ value, unit, dirty }) => (
  <div className={clsx(valueBoxCls, 'flex items-baseline gap-[6px]')}>
    <span className={clsx('font-extrabold text-[14.5px]', dirty ? 'text-[#b45309]' : 'text-[#0f172a]')}>{formatNumberVN(value)}</span>
    {unit && <span className="text-[13px] font-bold text-[#64748b]">{unit}</span>}
  </div>
);
    ...
    <div className="flex flex-col">
      ...
      <div className="bg-surface border border-[#e5e7eb] rounded-[16px]">
        <div className="pt-[22px] px-[22px] pb-0">
          <h1 className="text-[26px] font-black text-[#0f172a] m-0 mb-[4px] tracking-[-0.3px] flex items-center gap-[10px]"><Settings size={22} /> Cấu hình giá</h1>
          <p className="text-[15px] text-muted m-0">Quản lý nguồn giá gốc và quy tắc tính giá bán cho hệ thống kho.</p>
          ...
          {activeTab === 'SOURCE' && (
            <>
              ...
              >
                ...
                {importResult && <div className="mb-[12px] text-[#16a34a] text-[14px] font-bold">{importResult}</div>}
                                  ...
                                  <td className={tdCls}>
                                    {isEditing ? (
                                      <div className="flex items-center gap-[8px]">
                                        <label className="flex items-center gap-[4px] text-[12px] text-[#6b7280] font-bold cursor-pointer shrink-0 whitespace-nowrap">
                                      ...
                                      isUnlimited ? <span className={clsx(valueBoxCls, 'font-extrabold text-[#0f172a] text-[16px]')}>Không giới hạn</span> : <ValueDisplay value={tier.maxCost} unit="VNĐ" />
                                    )}
                                  </td>
                                  <td className={tdCls}>
                                    {isEditing ? (
            ...
            <div className="border border-[#e5e7eb] rounded-[12px] p-[18px]">
              <div className="flex items-start justify-between mb-[14px]">
                <div className="flex items-center gap-[8px]">
                  ...
                  <div>
                    <h3 className="text-[15.5px] font-extrabold text-[#0f172a] m-0">Tiền công / VAT</h3>
                    <p className="text-[13px] text-[#64748b] m-0 mt-[2px]">Theo danh mục sản phẩm</p>
                  </div>
                </div>
                ...
                return (
                  <>
                    ...
                    {categories.length === 0 && !addingCategory && (
                      <div className="py-[10px] px-0 text-center text-[#64748b] text-[14px]">Chưa có danh mục sản phẩm nào</div>
                    )}
                    {totalPages > 1 && (
                      <div className="flex items-center justify-end gap-[8px] mt-[12px]">
                        ...
                        <span className="text-[13px] font-bold text-[#334155]">{safePage}/{totalPages}</span>
      ...
      {toast && (
        <div
          className={clsx(
            'fixed bottom-[20px] right-[20px] z-50 flex items-center gap-[8px] py-[12px] px-[16px] rounded-[10px] shadow-[0_10px_25px_rgba(0,0,0,0.18)] text-surface text-[14.5px] font-bold max-w-[360px]',
  ...
  return (
    <div className="flex flex-col">
        ...
        return (
          <div key={c.id} className={clsx('flex flex-col gap-[6px] py-[9px] px-0 border-b border-[#f1f5f9]', markedDelete ? 'bg-[#fef2f2]' : isDirty ? 'bg-[#fffbeb]' : '')}>
            <div className="flex items-center justify-between gap-[8px]">
              <span className={clsx('text-[14.5px] font-bold', markedDelete ? 'text-[#64748b] line-through' : 'text-[#334155]')}>{c.name}</span>
            ...
            <div className="flex items-center gap-[8px]">
              <div className="flex-1">
                <label className="text-[11.5px] text-[#64748b] font-bold block mb-[2px]">Tiền công</label>
                {isEditing && !markedDelete ? (
                  <MoneyField value={c.laborCost || 0} onChange={(v) => onLaborCostChange(c.id, v)} width="100%" />
                ) : (
                  <span className={clsx(valueBoxCls, 'block w-full box-border text-[14.5px] font-extrabold', isDirty ? 'text-[#b45309]' : 'text-[#0f172a]')}>{formatNumberVN(c.laborCost || 0)}</span>
                )}
              </div>
              <div className="w-[70px] shrink-0">
                <label className="text-[11.5px] text-[#64748b] font-bold block mb-[2px]">VAT</label>
                {isEditing && !markedDelete ? (
                  <PercentField value={c.vatRate || 0} onChange={(v) => onVatRateChange(c.id, v)} width="100%" />
                ) : (
                  <span className={clsx(valueBoxCls, 'text-[14.5px] font-extrabold', isDirty ? 'text-[#b45309]' : 'text-[#0f172a]')}>{c.vatRate || 0}%</span>
                )}
              </div>
            </div>
          </div>
      ...
      {items.length === 0 && !adding && (
        <div className="py-[10px] px-0 text-center text-[#cbd5e1] text-[14px]">—</div>
      )}
      {adding && (
...
const StoneGroupTable: React.FC<{
  ...
  return (
    <div>
      <h4 className="text-[14px] font-extrabold text-[#334155] m-0 mb-[8px]">{title}</h4>
            ...
            <tfoot>
              <tr className={pcpAddRowCls}>
                <td colSpan={5} className="p-[8px]">
                  <button type="button" onClick={onOpenAdd} className="text-[#334155] hover:text-primary flex items-center justify-center gap-[6px] w-full bg-transparent border-0 cursor-pointer text-[14px] font-bold p-[4px]">
      ...
      {items.length > STONE_PAGE_SIZE && (
        <div className="flex items-center justify-end gap-[8px] mt-[10px]">
          ...
          <span className="text-[13.5px] font-bold text-[#334155]">Trang {safePage}/{totalPages}</span>
