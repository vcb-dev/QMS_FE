    ...
    <div className="flex flex-col gap-[18px] pb-[30px]">
      ...
      <div className="grid [grid-template-columns:1.6fr_1fr] gap-[20px] items-start max-[960px]:!grid-cols-1">
        ...
        <div className="flex flex-col gap-[16px]">
          ...
          <div className={cardCls}>
            ...
            <div className={clsx(formGroupCls, 'mb-[16px]')}>
              <label className={clsx(formLabelCls, 'text-[14px] font-extrabold text-[#334155] uppercase block mb-[6px]')}>
                DANH MỤC SẢN PHẨM
              </label>
              <select
                value={categoryId}
                onChange={(e) => setCategoryId(e.target.value)}
                className="w-full py-[10px] px-[14px] rounded-[8px] border border-[#cbd5e1] text-[16px] font-bold outline-none bg-surface"
              >
            ...
            <div className="border border-[#e2e8f0] rounded-[12px] p-[14px] bg-[#f8fafc]">
              ...
              >
                <span className="text-[14px] font-extrabold text-[#334155] uppercase">
                  LOẠI VÀNG / CHẤT LIỆU
                </span>
                <span className="text-[14px] font-extrabold text-[#334155] uppercase">
                  TRỌNG LƯỢNG (CHỈ)
                ...
                {materialRows.map((row) => (
                  ...
                  >
                    <select
                      ...
                      className="w-full py-[9px] px-[12px] rounded-[8px] border border-[#cbd5e1] text-[16px] font-bold outline-none bg-surface"
                    >
                      {dbMaterials
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full pt-[9px] pr-[42px] pb-[9px] pl-[12px] rounded-[8px] border border-[#cbd5e1] text-[16px] font-bold outline-none bg-surface [font-variant-numeric:tabular-nums]"
                      />
                      <span className="absolute right-[12px] top-1/2 -translate-y-1/2 text-[15px] font-bold text-[#334155]">
                        chỉ
                      </span>
                    </div>
              ...
              {materialRows.length > 1 && (
                <div className="flex justify-between items-baseline pt-[10px] mt-[10px] border-t border-dashed border-[#cbd5e1] text-[15px] text-[#475569]">
                  <span className="font-bold">Tổng khối lượng:</span>
                  <strong className="text-[16px] text-[#0f172a] [font-variant-numeric:tabular-nums]">
              ...
              <div className="flex items-center justify-between mb-[10px]">
                ...
                {!autoGoldMode && (
                  <button
                    ...
                    className="flex items-center gap-[6px] bg-surface border border-[#cbd5e1] rounded-[8px] text-[#334155] text-[15px] font-extrabold py-[6px] px-[12px] cursor-pointer"
                  >
                ...
                <div className="flex flex-col gap-[8px]">
                  {compareRows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        {autoGoldMode ? (
                          <span className="py-[9px] px-[12px] text-[16px] font-bold text-[#334155]">{row.materialName}</span>
                        ) : (
                          <select
                            value={row.materialId || ''}
                        ...
                        <div className="relative">
                          <input
                            ...
                            placeholder="Số chỉ"
                            className={clsx('w-full pt-[9px] pr-[42px] pb-[9px] pl-[12px] rounded-[8px] text-[16px] font-bold outline-none bg-surface [font-variant-numeric:tabular-nums]', !autoGoldMode && missingWeight ? 'border border-[#f87171]' : 'border border-[#cbd5e1]')}
                          />
                          <span className="absolute right-[12px] top-1/2 -translate-y-1/2 text-[15px] font-bold text-[#334155]">
                            chỉ
                          </span>
            ...
            {isSale && (
              <label className="flex items-center gap-[8px] mt-[14px] pt-[14px] border-t border-dashed border-[#e2e8f0] cursor-pointer w-fit">
                ...
                />
                <span className="text-[16px] font-bold text-[#374151]">Cộng VAT vào báo giá</span>
              </label>
            )}
            ...
            {isSilverMaterial && !isSale && (
              <div className="mt-[14px] pt-[14px] border-t border-dashed border-[#e2e8f0] flex items-center gap-[10px]">
                <label className="text-[14px] font-extrabold text-[#334155] uppercase">Hệ số nhân Bạc</label>
                <select
                  value={selectedSilverMultiplier}
                  onChange={(e) => setSelectedSilverMultiplier(parseFloat(e.target.value) || 0)}
                  className="py-[8px] px-[12px] rounded-[8px] border border-[#cbd5e1] text-[16px] font-bold outline-none bg-surface"
                >
                ...
                {baseMetals.map((m, idx) => (
                  <React.Fragment key={m.id}>
                    ...
                    <div className="flex items-baseline gap-[10px] flex-wrap">
                      <span className="text-[17px] font-extrabold text-[#475569] uppercase tracking-[0.3px]">
                        GIÁ {m.name.toUpperCase()}
                      </span>
                      <strong className="text-[22px] font-black text-[#334155] tracking-[0.3px] [font-variant-numeric:tabular-nums]">
                        {formatNumberVN(m.priceVnd)} <span className="text-[17px] font-bold">đ/chỉ</span>
                      </strong>
                    </div>
                  </React.Fragment>
                ))}
            ...
            <div className={clsx('grid grid-cols-2 gap-[18px] max-[960px]:!grid-cols-1', isSale && '![grid-template-columns:1fr]')}>
              ...
              <div className={cardCls}>
                <div className="flex items-center justify-between mb-[14px]">
                  <h3 className="text-[18px] font-extrabold text-[#0f172a] m-0">Thông số Đá quý</h3>
                  {stoneInputMode === 'table' && (
                    <button
                      type="button"
                      onClick={addStoneRow}
                ...
                <div className="flex gap-[8px] mb-[14px]">
                  <button
                    ...
                    className={clsx('flex-1 py-[8px] px-[10px] rounded-[8px] text-[15px] font-extrabold cursor-pointer', stoneInputMode === 'table' ? 'border-[1.5px] border-[#0f172a] bg-[#fff7ed] text-[#c2410c]' : 'border border-[#cbd5e1] bg-surface text-muted')}
                  >
                    Tính từ bảng đá
                  </button>
                  <button
                    ...
                    className={clsx('flex-1 py-[8px] px-[10px] rounded-[8px] text-[15px] font-extrabold cursor-pointer', stoneInputMode === 'total' ? 'border-[1.5px] border-[#0f172a] bg-[#fff7ed] text-[#c2410c]' : 'border border-[#cbd5e1] bg-surface text-muted')}
                  >
                    Nhập tổng tiền đá
                  </button>
                </div>
                ...
                {stoneInputMode === 'total' ? (
                  ...
                  <div className="flex flex-col gap-[12px]">
                      ...
                      >
                        <div className="flex items-center justify-end mb-[6px]">
                          <button
                            ...
                            className="bg-transparent border-0 text-[#ef4444] cursor-pointer text-[19px] font-extrabold"
                          >
                            ✕
                          </button>
                        </div>
                        ...
                        <div className="flex flex-col gap-[12px]">
                          <div className="grid grid-cols-2 gap-[10px]">
                            <div>
                              <label className="text-[13.5px] text-[#334155] font-bold block mb-[4px]">
                                Loại đá
                              </label>
                              <select
                                value={row.stoneType}
                                onChange={(e) => updateStoneRow(row.id, { stoneType: e.target.value as StoneRow['stoneType'], stoneId: '' })}
                                className="w-full py-[8px] px-[10px] rounded-[6px] border border-[#cbd5e1] text-[15.5px] font-semibold bg-surface"
                              >
                            ...
                            <div>
                              ...
                              <select
                                ...
                                className={clsx('w-full py-[8px] px-[10px] rounded-[6px] border border-[#cbd5e1] text-[15.5px] font-semibold', row.stoneType ? 'bg-surface' : 'bg-[#f1f5f9]')}
                              >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="w-full py-[8px] px-[10px] rounded-[6px] border border-[#cbd5e1] text-[15.5px] font-bold bg-surface text-right"
                            />
                          </div>
                    ...
                    <div className="flex justify-between items-baseline pt-[10px] pr-[4px] pb-0 pl-[4px]">
                      <span className="text-[15px] font-extrabold text-[#374151]">Tổng tiền đá</span>
                      <strong className="text-[18px] text-[#c2410c] [font-variant-numeric:tabular-nums]">
                        {calcResult?.stoneCost != null ? formatCurrency(calcResult.stoneCost) : '—'}
                      </strong>
                    </div>
                  </div>
                ) : (
                  <div className="w-full p-[24px] rounded-[12px] border border-dashed border-[#cbd5e1] bg-[#f8fafc] text-center text-[#334155] text-[16px]">
              ...
              {!isSale && (
                <div className={cardCls}>
                  <h3 className="text-[18px] font-extrabold text-[#0f172a] mt-0 mr-0 mb-[16px] ml-0">Chế tác & Phí dịch vụ</h3>
          ...
          {priceOptions.length > 0 && (
            <div className={cardCls}>
              <div className="flex items-center justify-between mb-[16px]">
                <h3 className="text-[18px] font-extrabold text-[#0f172a] m-0">Các Phương Án Giá</h3>
                <button
                  type="button"
                  onClick={handleCopyAllPrices}
                  className={clsx('flex items-center gap-[6px] py-[7px] px-[14px] rounded-[8px] border border-[#cbd5e1] text-[15px] font-bold cursor-pointer', copiedAll ? 'bg-[#dcfce7] text-[#16a34a]' : 'bg-surface text-[#475569]')}
                >
                ...
                {priceOptions.map((opt, idx) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="text-[14.5px] font-bold text-[#334155] overflow-hidden text-ellipsis whitespace-nowrap">
                        {cleanOptionLabel(opt)}
                      </div>
                      <div className="flex flex-col">
                        <div className="text-[19px] font-black text-[#0f172a] mt-[2px] [font-variant-numeric:tabular-nums]">
        ...
        >
            ...
            <>
              ...
              <div className="bg-surface border border-[#e5e7eb] rounded-[14px] p-[16px] flex flex-col gap-[9px]">
                {calcResult && (currentRole === 'ORDER' || currentRole === 'ADMIN') && (
                  <>
                    {calcResult.breakdown && calcResult.breakdown.length > 1 ? (
                      <div className="bg-[#f8fafc] py-[8px] px-[10px] rounded-[8px] mb-[4px] text-[14.5px] border border-[#e2e8f0]">
                ...
                <div className="flex items-baseline justify-between">
                  <span className="text-[15.5px] font-extrabold text-[#374151]">Tổng báo giá</span>
                  <div className="flex flex-col items-end">
                    <span className="text-[27px] font-black text-[#0f172a] [font-variant-numeric:tabular-nums]">
              ...
              <div className="w-full flex flex-col gap-[10px] mt-[4px]">
                ...
                <button
                  ...
                  onClick={handleCalculate}
                  disabled={isCalculating}
                  className={clsx('w-full bg-surface text-[#4b5563] border border-[#e5e7eb] rounded-[12px] p-[12px] text-[16px] font-bold flex items-center justify-center gap-[6px] transition-[all_0.2s_ease]', isCalculating ? 'cursor-not-allowed opacity-70' : 'cursor-pointer opacity-100')}
                >
                  <RotateCcw size={14} />
          ...
          {errorMessage && (
            <div className="text-[#b91c1c] text-[15px] bg-[#fef2f2] border border-[#fca5a5] p-[10px] rounded-[8px] w-full">
               {errorMessage}
            </div>
          )}
        </div>
...
const BreakdownRow: React.FC<{ label: string; value: number; accent?: string }> = ({ label, value, accent }) => (
  <div className="flex items-baseline justify-between">
    ...
    <span
      className={clsx('text-[16px] font-bold [font-variant-numeric:tabular-nums]', !accent && 'text-[#374151]')}
      // động — giữ inline
      style={accent ? { color: accent } : undefined}
    >
