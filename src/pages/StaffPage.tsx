  ...
  return (
    <div className="flex flex-col gap-[22px]">
      <div>
        ...
        <p className="text-[17px] text-muted m-0 mt-[4px]">
      ...
      <div className="bg-surface border border-border rounded-[12px] py-[10px] px-[14px] flex items-center gap-[12px] flex-wrap">
        <span className="text-[15px] font-extrabold text-muted uppercase tracking-[0.4px] inline-flex items-center gap-[6px]">
        ...
        <div className="flex items-center gap-[6px]">
          ...
          />
          <span className="text-faint text-[16px]">—</span>
          <input
            type="date"
            value={endDate}
        ...
        {isFiltered && (
          <button
            ...
            className="inline-flex items-center gap-[4px] py-[6px] px-[12px] rounded-[6px] bg-surface border border-border text-[#475569] text-[15.5px] font-bold cursor-pointer"
          >
            <RotateCcw size={12} /> Xóa lọc
          </button>
        )}

        {loading && <span className="text-[15.5px] text-faint font-semibold">Đang tải…</span>}
      </div>
      ...
      <div className="grid grid-cols-4 gap-[16px]">
        ...
        <div className="bg-surface border border-border rounded-[14px] py-[18px] px-[20px] shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
          <div className="text-[15px] font-extrabold text-muted uppercase">Sale / Order</div>
          <div className="text-[22px] font-black text-[#0f172a] mt-[8px]">
      ...
      <div className={cardContainerCls}>
        ...
        <span className="text-[15px] text-muted">Duyệt tài khoản mới hoặc khóa/mở khóa tài khoản đang hoạt động</span>
        ...
        <div className="flex gap-[8px] mt-[14px] border-b border-[#f1f5f9] pb-[12px]">
          <button
            ...
            className={clsx(
              'inline-flex items-center gap-[6px] py-[7px] px-[14px] rounded-[8px] text-[16.5px] font-bold cursor-pointer border',
                  ...
                  <tr key={u.id} className="border-b border-[#f8fafc]">
                    ...
                    <td className="p-[10px] text-[#334155]">
                      <select
                        ...
                        className="py-[5px] px-[8px] rounded-[6px] border border-border bg-surface text-[#334155] text-[15.5px] font-semibold cursor-pointer"
                      >
                    ...
                    <td className="p-[10px] text-right">
                      <div className="inline-flex gap-[6px]">
                        <button
                          ...
                          className={clsx(
                            'inline-flex items-center gap-[4px] py-[5px] px-[10px] rounded-[6px] border-0 bg-[#16a34a] text-surface text-[15.5px] font-bold',
                        ...
                        <button
                          ...
                          className={clsx(
                            'inline-flex items-center gap-[4px] py-[5px] px-[10px] rounded-[6px] border border-[#fecdd3] bg-[#fff1f2] text-[#be123c] text-[15.5px] font-bold',
                            actionLoadingId === u.id ? 'cursor-default opacity-60' : 'cursor-pointer',
                          )}
                        >
                          <X size={12} /> Từ chối
        ...
        {accountTab === 'ACTIVE' && (activeListUsers.length > 0 ? (
              ...
              <tbody>
                {pagedAccountList.map((u) => (
                  <tr key={u.id} className="border-b border-[#f8fafc]">
                    ...
                    <td className="p-[10px]">
                      <span className={clsx(
                        'inline-flex items-center gap-[4px] py-[3px] px-[8px] rounded-[20px] text-[15px] font-bold border',
                    ...
                    <td className="p-[10px] text-right">
                      <button
                        ...
                        className={clsx(
                          'inline-flex items-center gap-[4px] py-[5px] px-[10px] rounded-[6px] text-surface text-[15.5px] font-bold',
          ...
          <div className="text-center text-faint text-[16.5px] py-[20px] px-0">Chưa có tài khoản nào được duyệt</div>
        ))}
      ...
      <div className={cardContainerCls}>
        <h2 className="text-[18px] font-extrabold text-[#0f172a] m-0 mb-[14px]">Phân bố theo bộ phận</h2>
        {deptStats.length > 0 ? (
          <div className="flex flex-col gap-[10px]">
            {deptStats.map(([name, count]) => (
              <div key={name} className="flex items-center gap-[12px]">
                <span className="text-[16.5px] text-[#334155] font-semibold w-[160px] shrink-0 overflow-hidden text-ellipsis whitespace-nowrap">{name}</span>
                ...
                <span className="text-[16.5px] font-black text-[#0f172a] w-[24px] text-right shrink-0">{count}</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="text-center text-faint text-[16.5px] py-[20px] px-0">Chưa có dữ liệu</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-[16px]">
        <div className={cardContainerCls}>
          ...
          <span className="text-[15px] text-muted">Số yêu cầu đã tạo & đã chốt của từng Sale</span>
        ...
        <div className={cardContainerCls}>
          ...
          <span className="text-[15px] text-muted">Thời gian trung bình báo giá & xử lý của từng Order</span>
        ...
        {Object.keys(actionStats).length > 0 ? (
          ...
          >
            {Object.entries(actionStats).map(([role, rawActions]) => {
              ...
              return (
                <div key={role}>
                  <div className="text-[15px] font-extrabold text-muted uppercase mb-[10px]">
                      ...
                      return (
                        <div key={a.action}>
                          <div
                            ...
                            className="flex justify-between text-[15.5px] text-[#334155] mb-[3px] cursor-pointer"
                          >
                          ...
                          {isOpen && (
                            <div className="mt-[6px] py-[8px] px-[10px] bg-[#f8fafc] border border-[#f1f5f9] rounded-[8px] flex flex-col gap-[4px]">
                              {a.byActor.filter((actor) => !actor.actorId || !lockedUserIds.has(actor.actorId)).map((actor) => (
                                <div key={actor.actorId || actor.actorName} className="flex justify-between text-[15px]">
